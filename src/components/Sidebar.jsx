...
export default function Sidebar() {
    const menuItems = [
        { id: 'Dashboard', label: '대시보드', icon: LuLayoutDashboard, path:'/' },
        { id: 'Review', label: '리뷰 관리', icon: LuStar, path:'/review' },
        { id: 'Notice', label: '공지 사항', icon: LuBell, path:'/notice' },
        { id: 'Faq', label: 'FAQ', icon: LuCircleHelp, path:'/faq' }
    ]
            ...
            {menuItems.map((item)=>{
                ...
                return (
                    <NavLink
                        ...
                        to={item.path}
                        className={({ isActive }) =>
                            `w-full flex items-center gap-3 px-3 py-2.5 mb-1 rounded-md text-sm transition-colors ${
                                isActive 
